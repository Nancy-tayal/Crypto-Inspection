...
top_data['Date'] = pd.to_datetime(top_data['Date']).dt.strftime("%d-%m-%Y")
...
fig, ax1 = plt.subplots(5, figsize=(20,17))
...
coindf1.plot(figsize=(10,7)) # Data won't be presented very well as they are having different scales
plt.show()
...
df2.dropna(inplace=True)
fig, ax1 = plt.subplots(5, figsize=(15,10))
...
df = pd.DataFrame({'BTC':bit_coin["Close"],'ETH':Ethereum_coin["Close"], 'XRP':XRP_coin["Close"],'ADA':Cardano_coin["Close"],'XLM':Stellar_coin["Close"]}, index=bit_coin.index)
c=['r','b','g','y','m']
title = ['Bitcoin','Ethereum','XRP','Cardano','Stellar']
for i in range(5):
  print(' '*25+title[i])
  s=seasonal_decompose(df.iloc[:,i])
  s.plot()
  plt.show()
...
df = pd.read_csv('/content/coin_XRP-long.csv')
...
df=pd.read_csv('top_Five.csv')
bit_coin=df.loc[df['Name'].str.contains("Bitcoin", case=False)].reset_index(drop=True)
Ethereum_coin = df.loc[df['Name'].str.contains("Ethereum", case=False)].reset_index(drop=True)

bit_coin['Close'].plot(label = 'Bitcoin', figsize=(10,8))
Ethereum_coin['Close'].plot(label = 'Ethereum', figsize=(10,8))
plt.legend()

bit_coin['Cumu'] = bit_coin['Close']/bit_coin['Close'].iloc[0]
Ethereum_coin['Cumu'] = Ethereum_coin['Close']/Ethereum_coin['Close'].iloc[0]

bit_coin['Cumu'].plot(label = 'Bitcoin', figsize=(10,8))
Ethereum_coin['Cumu'].plot(label = 'Ethereum')
plt.legend()

bit_coin['daily_ret'] = bit_coin['Close'].pct_change(1)
Ethereum_coin['daily_ret'] = Ethereum_coin['Close'].pct_change(1)
plt.scatter(bit_coin['daily_ret'],Ethereum_coin['daily_ret'])

LR = stats.linregress(bit_coin['daily_ret'].iloc[1:],Ethereum_coin['daily_ret'].iloc[1:])
LR
